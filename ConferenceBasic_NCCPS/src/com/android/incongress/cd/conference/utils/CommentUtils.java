...
//	        MyLinkify.addLinks(view, MyLinkify.WEB_URLS);
    ...
    public static  void addAnswerLinks(int color, String name, TextView view) {
        MyLinkify.TransformFilter mentionFilter = new MyLinkify.TransformFilter() {
            public final String transformUrl(final Matcher match, String url) {
                return match.group(1);
            }
        ...
        if(!TextUtils.isEmpty(name)){
            int end=name.length();
            value.setSpan(new ForegroundColorSpan(color), view.getText().length()-end, view.getText().length(),Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        view.setText(value);
    }

    public static  void addCertainPositionLinks(int color, String firstName, String secondName , TextView view) {
        MyLinkify.TransformFilter mentionFilter = new MyLinkify.TransformFilter() {
            public final String transformUrl(final Matcher match, String url) {
                return match.group(1);
            }
        ...
        if(!TextUtils.isEmpty(firstName) && !TextUtils.isEmpty(secondName)){
            int fisrtLength = firstName.length();
            int secondeLength = secondName.length();
            value.setSpan(new ForegroundColorSpan(color), fisrtLength + 2, fisrtLength + 2 + secondeLength,Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        view.setText(value);
    }
}
